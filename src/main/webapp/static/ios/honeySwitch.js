	...
	$("[class^=switch]").click(function() {
		...
		if ($(this).hasClass("switch-on")) {
			...
			$.ajax({
				...
				type: 'get', //HTTP请求类型
				url: ctx+"/huiyuanGL/close?id="+this.id,
				success:function(result){
					console.log("关闭");
					mythis.removeClass("switch-on").addClass("switch-off");
					$(".switch-off").css({
						'border-color' : '#dfdfdf',
			...
			$.ajax({
				dataType: 'json', //服务器返回json格式数据
				type: 'get', //HTTP请求类型
				url: ctx+"/huiyuanGL/open?id="+this.id,
				success:function(result){
					mythis.removeClass("switch-off").addClass("switch-on");
					console.log("开启");
					...
					if (mythis.attr('themeColor')) {
						var c2 = mythis.attr('themeColor');
						mythis.css({
							'border-color' : c2,
